...
- **.env.local** *(не коммитится)*  
  Локальные настройки для разработки (тестовый бот + локальная БД).

- **.env.local.example**  
  Шаблон для локального `.env.local`.
...
- **docker-compose.local.yml**  
  Локальная БД для разработки (Postgres).
...
- **bot/features/**  
  Фичи бота, каждая в своей папке:
  - `onboarding/` — старт, присоединение, создание бюджета
  - `budgets/` — бюджеты, участники, активный бюджет
  - `main_menu/` — главное меню
  Внутри каждой фичи: `router.py`, `keyboards.py`, при необходимости `states.py`.
...
Назначение: сценарии и правила предметной области, вызываемые из feature-роутеров.
...
- **docs/local_dev.md** — локальная разработка (БД, окружение, запуск)
