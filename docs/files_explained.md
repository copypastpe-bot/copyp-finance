...
**Что важно:**
- `telegram_user_id` — уникальный ID Telegram пользователя
...
### `bot/features/`
**Зачем:**
- Группировка логики по фичам (onboarding, budgets, main_menu)
- Внутри одной фичи лежат и роутеры, и клавиатуры, и состояния

**Если разнести по типам файлов:**
- Сценарий расползётся по разным папкам
- Дольше искать логику одной фичи
...
**Зачем:**
- Отделяет правила и сценарии от Telegram слоя
- Дает единое место для работы с БД и доменными правилами

**Если убрать / сломать:**
- Вся логика расползётся по хендлерам
- Усложнится поддержка и тестирование
